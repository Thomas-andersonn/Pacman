...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        allPosition = util.Counter()
        if noisyDistance == None:
            allPosition[self.getJailPosition()] = 1.0
        else:
            for position in self.legalPositions:
                actualDistance = util.manhattanDistance(position, pacmanPosition)
                allPosition[position] = emissionModel[actualDistance] * self.beliefs[position]

        "*** END YOUR CODE HERE ***"

        allPosition.normalize()
        self.beliefs = allPosition
        allPossible=allPosition
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for previousPosition in self.legalPositions:
            ...
            for current, probability in newPosDist.items():
                allPosition[current] += self.beliefs[previousPosition] * probability
